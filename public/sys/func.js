...
class FUN {
    ...
    async logIn(email, password) {
        ...
        if (!odp.status) {
            await this.parent.modal.alert(t.error || t.alert_title, t.error_login);
            await this.showWinLogin();
            return;
        }
        await this.parent.modal.alert(t.success || t.alert_title, t.success_login);
    ...
    async logOut() {
        ...
        if (!odp.status) {
           await this.parent.modal.alert(t.error || t.alert_title, t.error_logout);
            await this.showWinLogout();
            return;
        }
        await this.parent.modal.alert(t.success || t.alert_title, t.success_logout);
        await this.parent.restart();
    }
}
